...
`.split('\n')));
      ...
      console.log(JSON.stringify(logs.inputs, null, 2));
    });
...
`.split('\n')));
      ...
      expect(logs.inputs[0].format.bitrate).toBe(500);
    });
...
`.split('\n')));
    ...
    it('should parse chapter start and end', async () => {
      const logs = await parseLogs(Readable.from(`\
[info] Input #0, matroska,webm, from 'video.mkv':
[info]   Duration: N/A, start: 0.000000, bitrate: N/A
[info]     Chapter #0:0: start 0.000000, end 30.000000
`.split('\n')));
      ...
      expect(logs.inputs[0].chapters[0].start).toBe(0);
      expect(logs.inputs[0].chapters[0].end).toBe(30);
    });
  });
});
